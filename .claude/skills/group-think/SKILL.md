...
| **Surface**      | Different surfaces have different stacks; Tamagui only lives in packages/rn-ui | "Which surface is affected — apps/web (Next.js + Tailwind), apps/wallet (Vite + react-native-web + shadcn), the perps routes in apps/wallet (just mount rn-ui screens, no Tamagui at this layer), packages/rn-ui/src/ itself (where Tamagui is authored), or a shared package?" |
