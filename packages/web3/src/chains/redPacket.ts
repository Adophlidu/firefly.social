...
import { visibleChains } from '@/chains/eth.js';

/**
 * Red-packet contract address for every chain where the contract is deployed.
 * This is the single source of truth — {@link rpSupportedChains} is derived from it,
 * and apps/web re-imports this map for contract lookups. To add red-packet support
 * for a chain, deploy the contract and add its address here (no other list to update).
 */
export const RED_PACKETS: Partial<Record<number, Address>> = {
...
/**
 * Chains where the UI allows creating/claiming a red packet: the intersection of
 * {@link visibleChains} (wallet-visible) and {@link RED_PACKETS} (contract deployed).
 * Derived — never edit by hand.
 */
export const rpSupportedChains = visibleChains.filter((chain) => RED_PACKETS[chain.id]);
