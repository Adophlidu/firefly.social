...
- Resolve the shared config path through the package export: `const config = createRequire(import.meta.url).resolve('@dimensiondev/rn-ui/tamagui.config')`. Then register `tamaguiPlugin({ optimize: true, disableResolveConfig: true, config, components: ['tamagui'] })` **before** `@vitejs/plugin-react`. Use `disableResolveConfig: true` when the app already aliases `react-native` / `react-native-svg` for RNW.
...
**Metro (native app):** use `@tamagui/babel-plugin` with `config` resolved via `require.resolve('@dimensiondev/rn-ui/tamagui.config')` (same export, same source `.ts` file) and `components: ['tamagui']` (and your design-system package name if applicable). Include `node_modules/@dimensiondev/rn-ui` in the Babel processing scope if you consume the prebuilt tarball instead of `src/`.
