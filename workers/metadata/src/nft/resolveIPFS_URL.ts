...
export function resolveIPFS_URL(cidOrURL: string | undefined): string | undefined {
    ...
    if (cidOrURL.startsWith(IPFS_GATEWAY_HOST)) {
        // base64 data string
        const [, data] = cidOrURL.match(MATCH_IPFS_DATA_RE) ?? [];
    ...
    if (MATCH_IPFS_CID_RE.test(cidOrURL)) {
        ...
        if (MATCH_IPFS_CID_AT_STARTS_RE.test(cidOrURL)) {
            try {
                ...
                if (cid) {
                    if (u.pathname === '/') {
                        return resolveIPFS_URL(urlcat('https://ipfs.io', `ipfs/${cid}`));
                    } else {
                        return resolveIPFS_URL(urlcat('https://ipfs.io', `/ipfs/${cid}/${u.pathname.slice(1)}`));
                    }
                }
            } catch (error) {
                console.log(`[resolveIPFS_URL] Error parsing URL: ${cidOrURL}`, error);
                // do nothing
            }
        }
