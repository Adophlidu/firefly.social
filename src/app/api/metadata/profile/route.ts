...
const SearchParamsSchema = z.object({
    ...
    preferByHandle: z.boolean().default(false),
});
...
export async function GET(request: NextRequest) {
    ...
    const { id, source, preferByHandle } = parsedParams.data;

    const provider = resolveSocialMediaProvider(resolveSocialSourceFromUrl(source));
    const profile = preferByHandle ? await provider.getProfileByHandle(id) : await provider.getProfileByIdOrHandle(id);
