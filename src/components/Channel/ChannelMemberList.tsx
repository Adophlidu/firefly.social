...
function getMemberInList(index: number, profile: Profile, listKey?: string) {
    return <ProfileInList profile={profile} key={`${profile.profileId}-${index}`} listKey={listKey} index={index} />;
}

export function ChannelMemberList({ channelId, source }: ChannelMemberListProps) {
    ...
    return (
        <ListInPage
            ...
            VirtualListProps={{
                key: `${ScrollListKey.ChannelMembers}:${source}:${channelId}`,
                computeItemKey: (index, item: Profile) => `${item.profileId}-${index}`,
                itemContent: (index, item: Profile) => getMemberInList(index, item),
            }}
