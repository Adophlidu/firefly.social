...
export function useSolanaAvailability(payload: RedPacketJSONPayload, chainId: number, enabled = true) {
    ...
    return useQuery({
        ...
        queryFn: async () => {
            const rpProgram = runInSafe(() => createRedPacketProgram(chainId));
            const accountId = resolveSolanaAccountId(payload.rpid, payload.accountId);

            if (!accountId || !rpProgram) return null;
            const data = await rpProgram.account.redPacket.fetch(accountId);
