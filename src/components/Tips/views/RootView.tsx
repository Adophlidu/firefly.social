...
export function RootView() {
    ...
    const { isSending, hash, hasError } = TipsContext.useContainer();
    ...
    const hideTitle = (isSending && !hasError && !!hash) || (!isSending && hasError);
    ...
    return (
        <div
            className={classNames('flex w-full flex-col transition-all', {
                'h-[358px] md:h-[368px]': [
                    TipsRoutePath.TIPS,
                    TipsRoutePath.SELECT_RECIPIENT,
                    TipsRoutePath.NO_AVAILABLE_WALLET,
                ].includes(pathname as TipsRoutePath),
