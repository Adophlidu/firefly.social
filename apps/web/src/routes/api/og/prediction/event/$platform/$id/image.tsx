...
import { getOgSatoriFonts, loadImageDataUri, type OgAssets } from '@/services/og/loadOgAsset.js';
...
async function loadPredictionEventOgImages(assets: OgAssets) {
    const [fireflyLogo, fallbackImage] = await Promise.all([
        loadImageDataUri(assets, '/image/simple-firefly-logo.png'),
        loadImageDataUri(assets, '/image/firefly-light-avatar.png'),
    ]);
    return { fireflyLogo, fallbackImage };
}

type PredictionEventOgImages = Awaited<ReturnType<typeof loadPredictionEventOgImages>>;
...
async function GameMarketOgImage({
    gameData,
    images,
}: {
    gameData: GameMarketOgData;
    images: PredictionEventOgImages;
}) {
...
async function PredictionEventOgImage({
    ...
    if (gameData) return GameMarketOgImage({ gameData, images });

    const eventImage = await fetchImageAsBase64(event.image, images.fallbackImage);
...
const getHandler = async (request: NextRequest, context?: NextRequestContext, env?: OgEnv) => {
    ...
    if (!id) return createProxyImageResponse(getDefaultOgImageUrl(), (path) =>
    ...
    if (!event) return createProxyImageResponse(getDefaultOgImageUrl(), (path) =>
        env!.ASSETS.fetch(new Request(new URL(path, request.url))),
    );
    ...
    const [sharerHandle, images] = await Promise.all([
        ...
        loadPredictionEventOgImages(assets),
    ]);

    return createOgImageResponse(await PredictionEventOgImage({ event, platform, sharerHandle, images }), {
        width: 1200,
        height: 630,
        fonts: await getOgSatoriFonts(['Inter', 'NotoSansSC'], new URL(request.url).origin, assets),
