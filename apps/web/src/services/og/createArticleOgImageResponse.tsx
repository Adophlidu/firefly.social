...
export async function loadArticleOgImages(assets: OgAssets) {
    const [background, wallet, fallbackAvatar] = await Promise.all([
    ...
    return { background, wallet, fallbackAvatar };
}

export type ArticleOgImages = Awaited<ReturnType<typeof loadArticleOgImages>>;
...
async function ArticleOpenGraphImage({
    article,
    coverUrl,
    images,
}: ArticleOpenGraphImageProps & { images: ArticleOgImages }) {
    ...
    const avatarBase64 = await fetchImageAsBase64(article.author.avatar, images.fallbackAvatar);
...
/**
 * Worker-native counterpart of createArticleOpenGraphImageResponse (which uses
 * next/og): renders through createOgImageResponse with assets inlined via the
 * ASSETS binding.
 */
export async function createArticleOgImageResponse(
    props: ArticleOpenGraphImageProps & { images: ArticleOgImages },
    origin: string,
    assets: OgAssets,
) {
    ...
    return createOgImageResponse(await ArticleOpenGraphImage(props), {
        width: 1200,
        height: 630,
        fonts: await getOgSatoriFonts(getFontPreferences(title, excerpt, authorName), origin, assets),
