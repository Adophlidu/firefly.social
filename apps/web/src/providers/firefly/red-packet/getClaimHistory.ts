...
import type { ClaimHistoryResponse, RedPacketClaimListInfo } from '@/providers/types/FireflyRedPacket.js';
import { settings } from '@/settings/index.js';

export async function getClaimHistory(
    redpacket_id: string,
    indicator?: PageIndicator,
): Promise<RedPacketClaimListInfo> {
    const url = urlcat(settings.FIREFLY_ROOT_URL, '/v1/redpacket/claimHistory', {
        redpacketId: redpacket_id,
        cursor: indicator?.id,
        size: 20,
    });
    const { data } = await fetchJson<ClaimHistoryResponse>(url);
