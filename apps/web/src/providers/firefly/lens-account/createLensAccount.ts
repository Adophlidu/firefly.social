...
interface CreateLensAccountOptions {
    /** Lens username local name, must match /^[a-z0-9][a-z0-9_-]{4,}$/. */
    username: string;
    /** Display name for the Lens account metadata. */
    name?: string;
 ...
 * bind it on the server. The backend selects the Privy EVM wallet, deploys the
 * Lens account, and is idempotent (it returns an "already created" error when a
 * Lens account is already bound).
 */
