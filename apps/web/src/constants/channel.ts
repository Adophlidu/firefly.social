 ...
 *
 * Verified against Lens mainnet: group `0x230c140a85af16aa444ba87e0823e5c62cfe3366`
 * (metadata.name "WorldCup2026", 341 members). `feedId` is the group's *feed*
 * address `group.feed.address` — NOT the group address — because both the publish
 * path (`publishLensPost` → `feed`) and the posts query (`feeds: [{ feed }]`) take
 * the feed address. The group address itself returns zero posts.
 */
