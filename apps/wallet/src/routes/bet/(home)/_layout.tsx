...
export default function BetHomeLayout({ children }: { children?: ReactNode }) {
    const { pathname } = useRouterState();
    const navigate = useNavigate();
    const isHistoryPage = pathname === '/bet/history';
        ...
        return (
            <div className="flex w-full flex-1 flex-col items-center">
                <BetNavigationBar
                    ...
                    onBack={() => navigate('/bet', { replace: true })}
                />
...
function TabNavigation() {
    const navigate = useNavigate();
    const { pathname: pathnameInTab } = useRouterState();
    const tabs = [
